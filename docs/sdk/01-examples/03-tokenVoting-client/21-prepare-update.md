...
### Prepare the Update of the TokenVoting plugin

The `prepareUpdate` method performs the prior steps so that a DAO proposal can eventually apply the the update the TokenVoting plugin.
...
const prepareUpdateParams: TokenVotingPluginPrepareUpdateParams = {
